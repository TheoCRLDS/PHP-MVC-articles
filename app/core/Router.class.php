...
class Router
{
    ...
    public const URL_PATH = '/PHP_Vanilla/php-boilerplate-alt/';
    ...
    //permet d'appeler une URL en prenant en compte le lien du dossier physique
    public static function url($path, $method = 'GET')
    {
        $path = trim($path, '/');
        $path = $path ? '/' . $path : '';
        return $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . self::URL_PATH . $path;
    }
}
